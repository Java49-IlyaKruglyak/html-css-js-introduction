...
getLeastPopularMovie.innerHTML = leastPopularMovie();


function getPosterImages() {
    const arImages = moviesData.results.map(movie => {
        return createElement(movie);
    });
...
function createElement(movie) {
    ...
    return `<li class="movie-item"><img class="movie-image" src="${httpPrefix}${movie.poster_path}">
    <div class="frame">
    <lable class="original_title">${movie.original_title}</lable>
    <lable class="overview">${movie.overview}</lable>
    </div>
    </li>`;
}

function getMostPopular() {
    return moviesData.results.reduce((prev, curr) => {
        return prev.popularity > curr.popularity ? prev : curr;
    })
}

function mostPopularMovie() {
    return createElement(getMostPopular());
}

function getLeastPopular() {
    return moviesData.results.reduce((prev, curr) => {
        return prev.popularity < curr.popularity ? prev : curr;
    });
}
